...
__global__ void matMul(int N, _DOUBLE_ *C, _DOUBLE_ *A, _DOUBLE_ *B) {
    ...
    #pragma unroll 32  
        ...
        // go naive here..
        int I0 =  (2*blockIdx.y)*BLOCK_SIZE + threadIdx.y;
        int I1 =  (2*blockIdx.y+1)*BLOCK_SIZE + threadIdx.y;

        int J0 =  (2*blockIdx.x)*BLOCK_SIZE + threadIdx.x;
        int J1 =  (2*blockIdx.x+1)*BLOCK_SIZE + threadIdx.x;
